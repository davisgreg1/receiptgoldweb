...
function TeamAcceptContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { generateKeyPair, exportPublicKey, hybridDecrypt, hybridEncrypt } = await import('../../../lib/encryption');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-primary">
        <motion.div
          ...
          className="text-4xl text-gold-primary"
        >
          <FaSpinner />
        </motion.div>
      </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-background-primary">
        <motion.div
          ...
          className="max-w-md w-full text-center p-8 rounded-3xl bg-background-elevated shadow-large"
        >
          <div className="text-6xl mb-6 text-gold-primary">
            <FaReceipt />
          </div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => router.push('/')}
            className="px-6 py-3 rounded-2xl font-semibold bg-gold-primary text-text-inverse"
          >
            Go Home
          </motion.button>
        </motion.div>
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-background-primary">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          >
            <div
              className="w-20 h-20 rounded-2xl flex items-center justify-center relative shadow-large"
              style={{
                backgroundImage: `linear-gradient(135deg, var(--gold-primary), var(--gold-rich))`
              }}
            >
              <FaReceipt className="text-3xl text-text-inverse" />
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                className="absolute -top-1 -right-1"
              >
                <HiSparkles className="text-xl text-gold-primary" />
              </motion.div>
            </div>
          </motion.div>
          ...
          {/* Invitation Details */}
          <div className="p-6 rounded-2xl mb-8 bg-background-elevated shadow-medium">
            <div className="flex items-center justify-center mb-4">
              <FaBuilding className="text-2xl mr-3 text-gold-primary" />
              <div>
            ...
            <div className="inline-block px-4 py-2 rounded-full text-sm font-medium bg-gold-primary/20 text-gold-primary">
        ...
        <motion.div
          ...
          className="p-8 rounded-3xl bg-background-elevated shadow-large"
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                readOnly
                className="w-full px-4 py-3 rounded-xl opacity-60 bg-background-secondary text-text-primary border border-border-primary"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl focus:outline-none transition-all duration-300 bg-background-secondary text-text-primary border border-border-primary focus:border-gold-primary"
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2 text-text-primary">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  className="w-full px-4 py-3 pr-12 rounded-xl focus:outline-none transition-all duration-300 bg-background-secondary text-text-primary border border-border-primary focus:border-gold-primary"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 transform -translate-y-1/2 p-1 rounded-lg hover:bg-opacity-10 hover:bg-gray-500 transition-all duration-200 text-text-secondary"
                >
            ...
            {error && (
              <div
                className="p-4 rounded-xl text-sm border bg-status-error/10 text-status-error border-status-error/20"
              >
                {error}
              </div>
            )}
            ...
            <motion.button
              ...
              className={`w-full py-4 rounded-2xl font-semibold text-lg flex items-center justify-center space-x-2 transition-all duration-300 text-text-inverse shadow-medium ${(submitting || !formData.password || !formData.confirmPassword)
                  ? 'bg-gold-primary/60'
                  : 'bg-gold-primary'
                }`}
            >
          ...
          <p className="text-center text-sm mt-6 text-text-tertiary">
            By joining, you agree to our{' '}
            <Link
              href="/terms"
              className="underline hover:opacity-70 transition-opacity text-gold-primary"
            >
              Terms of Service
            </Link>
            {' '}and{' '}
